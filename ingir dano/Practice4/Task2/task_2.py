...
def main():
    file_name = 'task_2_var_56_subitem.text'

    data = parse_data(file_name)

    db = connect_to_db('task1.db')
    ...
    whrite_json(price_books, f'task_2_price_{name}.json')

    rating_top_price = get_rating_top_price(db)
    whrite_json(rating_top_price, 'task_2_top.json')

    avg_price = get_avg_price(db)
    whrite_json(avg_price, 'task_2_avg.json')
